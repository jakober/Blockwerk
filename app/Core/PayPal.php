...
class PayPal
{
    ...
    /**
     * Wie captureOrder, liefert aber zusätzlich den tatsächlich erfassten Betrag
     * (in Cent) und die Währung zurück – für serverseitige Betragsprüfung, damit
     * eine Freischaltung nur bei korrekt gezahltem Preis erfolgt.
     * @return array{ok:bool, amount:?int, currency:?string, error:?string}
     */
    public static function capture(string $orderId): array
    {
        $token = self::token();
        if ($token === null) {
            return ['ok' => false, 'amount' => null, 'currency' => null, 'error' => 'PayPal ist nicht korrekt konfiguriert.'];
        }
        [$res, $code] = self::request('POST', '/v2/checkout/orders/' . rawurlencode($orderId) . '/capture', $token, []);
        if ($res === null || ($code !== 200 && $code !== 201)) {
            return ['ok' => false, 'amount' => null, 'currency' => null, 'error' => 'Zahlung konnte nicht bestätigt werden.'];
        }
        $capture = $res['purchase_units'][0]['payments']['captures'][0]['amount'] ?? null;
        $amount = is_array($capture) ? (int) round(((float) ($capture['value'] ?? 0)) * 100) : null;
        $currency = is_array($capture) ? (string) ($capture['currency_code'] ?? '') : null;
        return [
