...
<div class="card narrow">
    <h2>Update-Quelle</h2>
    <p class="muted small">Liegt der Programmcode in einem <strong>privaten</strong> Repository, braucht der Updater einen Zugriffsschlüssel.
        Auf GitHub unter <em>Settings → Developer settings → Fine-grained tokens</em> einen Token mit Leserecht auf <em>Contents</em> für dieses Repository erzeugen und hier eintragen.
        Versions-Prüfung und Paket-Download laufen dann über die GitHub-API.</p>
    <form method="post" action="<?= e(url('/admin/update/quelle')) ?>">
        <?= csrf_field() ?>
        <div class="form-group">
            <label for="update_token">Zugriffsschlüssel <?= $tokenSet ? '(gespeichert – leer lassen zum Behalten)' : '' ?></label>
            <input type="password" id="update_token" name="update_token" value="" autocomplete="new-password" placeholder="<?= $tokenSet ? '••••••••' : 'github_pat_…' ?>">
        </div>
        <?php if ($tokenSet): ?>
            <div class="form-group checkbox-group">
                <label><input type="checkbox" name="update_token_clear" value="1"> Schlüssel entfernen (Repository ist wieder öffentlich)</label>
            </div>
        <?php endif; ?>
