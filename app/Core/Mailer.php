...
class Mailer
{
    ...
    {
        if (self::conf('mail_transport', 'mail') === 'smtp') {
    ...
    /**
     * Liest eine Mail-/SMTP-Einstellung aus den Einstellungen.
     */
    ...
    {
        $name = trim(self::conf('mail_from_name', ''));
        return $name !== '' ? $name : self::conf('site_name', 'Website');
    }
    ...
    {
        $host = trim(self::conf('smtp_host', ''));
        $port = (int) self::conf('smtp_port', '587');
        $encryption = self::conf('smtp_encryption', 'tls'); // none | ssl | tls (STARTTLS)
        $user = self::conf('smtp_user', '');
        $pass = self::conf('smtp_pass', '');
