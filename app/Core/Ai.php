...
/**
 * Client für den zentralen Blockwerk-Orange-KI-Dienst.
...
class Ai
{
    ...
    /** Kaufseite für KI-Guthaben (Token-Pakete) beim Anbieter. */
    public const DEFAULT_BUY_URL = 'https://blockwerk-orange.de/ki-guthaben';

    /**
     * Optionale Lizenz-Überschreibung für einzelne Anfragen (null = die in den
     * Einstellungen hinterlegte Lizenz der Installation).
     */
    private static ?string $overrideLicense = null;

    public static function useLicense(?string $key): void
    {
        self::$overrideLicense = ($key !== null && trim($key) !== '') ? trim($key) : null;
    }
    ...
    /**
     * Kaufseite für KI-Guthaben. Eigene Einstellung (ai_buy_url) hat Vorrang,
     * sonst die Standard-Kaufseite des Anbieters. So haben Standalone-Kunden
     * auch ohne eigene Konfiguration einen Weg, Guthaben zu kaufen/aufzuladen.
     */
    public static function buyUrl(): string
    {
        $url = trim(Setting::get('ai_buy_url', ''));
        return $url !== '' ? $url : self::DEFAULT_BUY_URL;
    }

    public static function licenseKey(): string
    {
        if (self::$overrideLicense !== null) {
            return self::$overrideLicense;
        }
    ...
    private static function request(string $method, string $path, array $body, int $timeout = 60): array
    {
        ...
        if ($status >= 400) {
            ...
            // HTTP-Status als Exception-Code mitgeben, damit Aufrufer den Fall
            // "kein Guthaben" (402) gezielt behandeln können (z. B. Bild ohne
            // Abbruch überspringen und auf Aufladen hinweisen).
