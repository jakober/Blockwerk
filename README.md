...
- **Layouts mit Design-Panel** – frei definierbare HTML-Grundgerüste mit Platzhaltern (`{{content}}`, `{{title}}`, `{{site_name}}`, `{{logo}}`, `{{menu}}`, `{{template:key}}`, …). Pro Layout wählbar: Grundfarben per Color-Picker (Primär-, Akzent-, Text-, Hintergrund- und Flächenfarbe) sowie Schriften für Überschriften und Fließtext – alle Inhaltselemente richten sich automatisch danach (CSS-Variablen `--cms-primary` usw.).
...
## KI-Assistent aktivieren (Lizenzschlüssel eintragen)

Der KI-Assistent erstellt Seiten, Texte und Bilder direkt im CMS. Er läuft über den zentralen Blockwerk-Dienst und rechnet über ein **Token-Guthaben** ab, das an deinen persönlichen **Lizenzschlüssel** (beginnt mit `bw-`) gebunden ist. Betreibst du das CMS selbst auf deinem Server (Standalone), aktivierst du die KI in drei Schritten:

1. **Guthaben kaufen.** Öffne die Kaufseite für KI-Guthaben beim Anbieter (Standard: <https://blockwerk-orange.de/ki-guthaben>), wähle ein Paket und zahle per PayPal. Deinen Schlüssel bekommst du **sofort per E-Mail** – zusammen mit der Rechnung.
2. **Schlüssel eintragen.** Im CMS-Backend unter **Einstellungen → KI-Assistent** den `bw-`-Schlüssel aus der E-Mail in das Feld *„Lizenzschlüssel"* einfügen. Die *Dienst-URL* leer lassen (Standard-Dienst). Speichern.
3. **Loslegen.** Unter **KI-Assistent** wird jetzt dein Guthaben angezeigt. Beschreibe einfach, was gebaut werden soll – die KI legt Seiten samt Texten und Bildern an.

**Guthaben aufladen:** Ist das Guthaben fast aufgebraucht, lädst du **denselben** Schlüssel auf der Kaufseite wieder auf – der Schlüssel bleibt gleich, nur das Guthaben steigt. Den Kontostand siehst du jederzeit oben rechts im KI-Assistenten.

> Guthaben-Pakete gibt es unter <https://blockwerk-orange.de/ki-guthaben> – dort kaufst du neu und lädst denselben Schlüssel jederzeit wieder auf.
