 ...
 *   {{logo}}           Logo der Website (Einstellungen) – leer, wenn keins gesetzt
...
class Renderer
{
    ...
    /**
     * Logo der Website ({{logo}}) aus den Einstellungen. Ist keins hinterlegt,
     * bleibt der Platzhalter leer – das Layout sieht dann aus wie zuvor.
     * So funktioniert ein Logo in JEDEM Layout, auch in klassischem HTML.
     */
    public static function logoImg(): string
    {
        $logo = trim((string) Setting::get('site_logo', ''));
        if ($logo === '') {
            return '';
        }
        return '<img class="site-logo" src="' . e($logo) . '" alt="'
            . e(Setting::get('site_name', 'Meine Website')) . '">';
    }
