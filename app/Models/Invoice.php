...
class Invoice
{
    ...
    public static function ensureTable(): void
    {
        ...
        try {
            ...
            // Gutschriften: Typ und Bezug zur ursprünglichen Rechnung.
            \Core\Database::ensureColumn(Database::pdo(), 'invoices', 'type', "VARCHAR(10) NOT NULL DEFAULT 'invoice'");
            \Core\Database::ensureColumn(Database::pdo(), 'invoices', 'ref_invoice_id', 'INT NULL');
            \Core\Database::ensureColumn(Database::pdo(), 'invoices', 'note', 'TEXT NULL');
        } catch (\Throwable) {
        }
        self::$ensured = true;
    }
     ...
     */
    public static function createForOrder(int $orderId, string $type = 'invoice', ?int $refInvoiceId = null, string $note = ''): array
    {
        self::ensureTable();
        ...
        $prefix = $isCredit
            ? (string) Setting::get('shop_credit_prefix', 'GS-')
            : (string) Setting::get('shop_invoice_prefix', 'RE-');
        ...
        for ($attempt = 0; $attempt < 5; $attempt++) {
            ...
            try {
                $stmt = $pdo->prepare('INSERT INTO invoices (order_id, seq, number, type, ref_invoice_id, note, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())');
                $stmt->execute([$orderId, $seq, $number, $isCredit ? 'credit' : 'invoice', $refInvoiceId, $note !== '' ? $note : null]);
                break;
            } catch (\Throwable) {
