...
/**
 * Vollständige URL inkl. Schema und Host – für E-Mails, PDFs und alles, was
 * außerhalb des Browsers landet. Dort helfen relative Pfade niemandem.
 */
function abs_url(string $path = ''): string
{
    $host = preg_replace('/^www\./i', '', (string) ($_SERVER['HTTP_HOST'] ?? ''));
    if ($host === '' || $host === null) {
        // Ohne Request-Kontext (z. B. Cron): hinterlegte Adresse nutzen.
        $host = preg_replace('#^https?://#', '', (string) \Models\Setting::get('site_url', '')) ?: '';
        $host = trim((string) $host, '/');
    }
    if ($host === '') {
        return url($path);
    }
    return \Core\App::scheme() . '://' . $host . url($path);
}
...
/**
 * Gibt die PHP-Session frühzeitig frei (schreibt & entsperrt die Session-Datei).
 * PHP sperrt die Session pro Request exklusiv – ein lange laufender Request (z. B.
 * eine KI-Anfrage) blockiert sonst ALLE weiteren Requests desselben Browsers, bis
 * er fertig ist (das Frontend „hängt"). Nach dem Aufruf bleibt $_SESSION lesbar,
 * ist aber nicht mehr persistent beschreibbar – daher erst nach allen Session-
 * Schreibzugriffen (Auth-Prüfung etc.) aufrufen.
 */
