...
class App
{
    ...
    /** Aufgerufener Host, normalisiert (klein, ohne Port, ohne führendes „www."). */
    ...
    /**
     * Läuft die Anfrage über HTTPS? Berücksichtigt auch den Betrieb hinter
     * einem TLS-Proxy (Caddy vor Apache/nginx): dort ist $_SERVER['HTTPS']
     * nicht gesetzt, wohl aber der Header X-Forwarded-Proto.
     */
    ...
    /** „https" oder „http" – für absolute URLs (Mails, Sitemap, Zahlungs-Rückläufer). */
    ...
    /** Startet die Session mit einem auf die Installation begrenzten Cookie-Pfad. */
    private function startSession(): void
    {
        ...
        session_set_cookie_params(['path' => $path !== '' ? $path : '/', 'httponly' => true, 'samesite' => 'Lax']);
        @session_start();
    }

    private function resolveBase(string $uriPath): void
    {
        // PHP-Built-in-Server (Entwicklung, `php -S -t public`): dort ist die
        // Docroot immer public/ → Basis leer. Zusätzlich setzt dieser Server
        // SCRIPT_NAME auf den vollen Request-Pfad, was die Ableitung unten bei
        // mehrsegmentigen URLs verfälschen würde. Echte Server (Apache/FPM) unten.
    ...
    private function registerRoutes(Router $router, bool $installed = true): void
    {
        ...
        $router->add('POST', '/admin/update/quelle', [\Controllers\Admin\UpdateController::class, 'saveSource']);
